...
test("manual social dialogs organize compact fields in category-specific rows", () => {
  assert.deepEqual(SOCIAL_MANUAL_LAYOUTS.status, [["society", 8], ["points", 2], ["level", 2], ["status_name", 5], ["monthly_cost", 7], ["description", 12]]);
  assert.deepEqual(SOCIAL_MANUAL_LAYOUTS.culture, [["culture_name", 8], ["points", 2], ["level", 2], ["description", 12]]);
  assert.deepEqual(SOCIAL_MANUAL_LAYOUTS.language, [["language_name", 10], ["points", 2], ["spoken_level", 6], ["written_level", 6], ["description", 12]]);
  ...
  assert.match(styles, /form-group--points/);
});
